...
# Education:
# Splits the years of education per person in four levels. 
# Based on the Section 2A data
# Uses the case_when function to split the years of education per person into 
# four levels: koranic_kinder_educ, prim_educ, sec_educ, educ_level.
...
edu_agg <- education %>% # general education @ individual level
  ...
  # edu_level gives a 3 for secondary or above, 
  # 2 for primary or above, 
  # 1 for koranic/kinder or above
  # 0 otherwise
  mutate(
    edu_level = case_when(
      sec_educ == 1 ~ 3,
