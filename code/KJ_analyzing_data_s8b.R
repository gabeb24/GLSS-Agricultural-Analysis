...
# Create base of HH that own and/or operate farm, or keep livestock, or engage in fishing
...
agri_land <- agri_plot_s8b %>%
  ...
  mutate(land_ropes = case_when(
      s8bq4b == 1 ~ s8bq4a * 9,  # convert acres to ropes (1:9)
      s8bq4b == 2 ~ s8bq4a * 9,  # convert acres to ropes (1:9)
  ...
  select(c(nh, clust, hh_land_ropes, region, ez, district, loc2, loc3, loc5))
...
#   koranic_kinder_educ, prim_educ, sec_educ, educ_level
...
hh_agri_edu_profit <- agri_land  %>%
  left_join(aggrev) %>%
  mutate(profit_per_rope = round((agri2c - hhagdepn) / hh_land_ropes, 2)) %>%
...
model_1 <- lm(data = hh_agri_edu_profit, profit_per_rope ~ loc2 + education_max)
summary(model_1)
